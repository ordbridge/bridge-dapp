...
# Framework Versions
ENV VERSION_NODE_18=18.18.2
ENV VERSION_NODE_DEFAULT=$VERSION_NODE_18
...
RUN /bin/bash -c ". ~/.nvm/nvm.sh && \
    nvm install $VERSION_NODE_18 && nvm use $VERSION_NODE_18 && \
    npm install -g yarn@${VERSION_YARN} sm grunt-cli bower vuepress gatsby-cli && \
    chown -R root:root /root/.nvm"

RUN /bin/bash -c ". ~/.nvm/nvm.sh && \
    nvm alias default ${VERSION_NODE_18} && nvm cache clear"

RUN echo "export PATH=$PATH:/root/.dotnet/tools:/root/.nvm/versions/node/${VERSION_NODE_18}/bin" >> ~/.bashrc && \
    echo "nvm use ${VERSION_NODE_18} 1> /dev/null" >> ~/.bashrc
